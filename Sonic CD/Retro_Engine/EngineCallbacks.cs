...
namespace Retro_Engine
{
	...
	public static class EngineCallbacks
	{
		...
		public static void PlayVideoFile(char[] fileName)
		{
			...
			Thread.Sleep(1000);
			#if WINDOWS_PHONE
			new MediaPlayerLauncher { Media = new Uri(text2, 2), Location = 1, Controls = 2 }.Show();
			#endif
		...
		public static void RetroEngineCallback(int callbackID)
		{
			...
			case 9:
				#if WINDOWS_PHONE
				new WebBrowserTask { Uri = new Uri("http://www.sega.com/legal/terms_mobile.php", 1) }.Show();
				#endif
				return;
			case 10:
				#if WINDOWS_PHONE
				new WebBrowserTask { Uri = new Uri("http://www.sega.com/legal/privacy_mobile.php", 1) }.Show();
				#endif
				return;
			case 14:
				if (!Guide.IsVisible)
		...
		public static void UpdateMessage(IAsyncResult ar)
		{
			...
			if (num != null)
			{
				...
				case 0:
				{
					...
					{
						#if WINDOWS_PHONE
						new MarketplaceDetailTask { ContentType = 1 }.Show();
						#endif
						return;
					}
					int num2 = 10;
