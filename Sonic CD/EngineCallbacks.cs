...
namespace Retro_Engine
{
    ...
    public static class EngineCallbacks
    {
      ...
      public static void PlayVideoFile(char[] fileName)
      {
        ...
        {
          Media = new Uri(uriString, UriKind.Relative),
          Location = ((MediaLocationType) 1),
          Controls = ((MediaPlaybackControls) 2)
        }.Show();
#endif
      ...
      public static void RetroEngineCallback(int callbackID)
      {
        switch (callbackID)
        {
          case 6:
            if (Guide.IsVisible)
              break;
            Guide.BeginShowMessageBox(EngineCallbacks.restartTitle[(int) GlobalAppDefinitions.gameLanguage], EngineCallbacks.restartMessage[(int) GlobalAppDefinitions.gameLanguage], (IEnumerable<string>) new string[2]
            {
            ...
            }, 1, MessageBoxIcon.Alert, new AsyncCallback(EngineCallbacks.ConfirmationScreen), (object) null);
            break;
          case 7:
            if (FileIO.activeStageList == (byte) 0)
            {
              if (Guide.IsVisible)
                break;
              Guide.BeginShowMessageBox(EngineCallbacks.exitTitle[(int) GlobalAppDefinitions.gameLanguage], EngineCallbacks.exitMessage[(int) GlobalAppDefinitions.gameLanguage], (IEnumerable<string>) new string[2]
              {
                EngineCallbacks.yesMessage[(int) GlobalAppDefinitions.gameLanguage],
                EngineCallbacks.noMessage[(int) GlobalAppDefinitions.gameLanguage]
              }, 1, MessageBoxIcon.Alert, new AsyncCallback(EngineCallbacks.ExitConfirmation), (object) null);
              break;
            }
            if (Guide.IsVisible)
              break;
