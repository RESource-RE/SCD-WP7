...
namespace Retro_Engine
{
    ...
    {
      public static Vertex3D[] vertexBuffer = new Vertex3D[4096 /*0x1000*/];
      public static Vertex3D[] vertexBufferT = new Vertex3D[4096 /*0x1000*/];
      public static Face3D[] indexBuffer = new Face3D[1024 /*0x0400*/];
      public static SortList[] drawList = new SortList[1024 /*0x0400*/];
      public static int numVertices = 0;
      public static int numFaces = 0;
      public static int projectionX = 136;
      public static int projectionY = 160 /*0xA0*/;
      public static int[] matWorld = new int[16 /*0x10*/];
      public static int[] matView = new int[16 /*0x10*/];
      public static int[] matFinal = new int[16 /*0x10*/];
      public static int[] matTemp = new int[16 /*0x10*/];
      ...
      public static void MatrixMultiply(ref int[] a, ref int[] b)
      {
        ...
        for (uint index1 = 0; index1 < 16U /*0x10*/; ++index1)
        {
          uint index2 = index1 & 3U;
          uint index3 = index1 & 12U;
          numArray[(int) index1] = (b[(int) index2] * a[(int) index3] >> 8) + (b[(int) (index2 + 4U)] * a[(int) (index3 + 1U)] >> 8) + (b[(int) (index2 + 8U)] * a[(int) (index3 + 2U)] >> 8) + (b[(int) (index2 + 12U)] * a[(int) (index3 + 3U)] >> 8);
        }
        for (uint index = 0; index < 16U /*0x10*/; ++index)
          a[(int) index] = numArray[(int) index];
      }
      ...
      {
        if (angleX < 0)
          angleX = 512 /*0x0200*/ - angleX;
        angleX &= 511 /*0x01FF*/;
        if (angleY < 0)
          angleY = 512 /*0x0200*/ - angleY;
        angleY &= 511 /*0x01FF*/;
        if (angleZ < 0)
          angleZ = 512 /*0x0200*/ - angleZ;
        angleZ &= 511 /*0x01FF*/;
      ...
      public static void TransformVertexBuffer()
      {
        ...
        for (int index4 = 0; index4 < Scene3D.numVertices; ++index4)
        {
      ...
      public static void Draw3DScene(int surfaceNum)
      {
        ...
        for (int index = 0; index < Scene3D.numFaces; ++index)
        {
          ...
          {
            case 0:
              if (Scene3D.vertexBufferT[face3D.a].z > 256 /*0x0100*/ && Scene3D.vertexBufferT[face3D.b].z > 256 /*0x0100*/ && Scene3D.vertexBufferT[face3D.c].z > 256 /*0x0100*/ && Scene3D.vertexBufferT[face3D.d].z > 256 /*0x0100*/)
              {
