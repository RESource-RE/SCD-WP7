...
namespace Sonic_CD
{
	...
	public class Game : Microsoft.Xna.Framework.Game
	{
		...
		protected void GetAchievementsCallback(IAsyncResult result)
		{
			...
			lock (this.achievementsLockObject)
			{
				...
				for (int i = 0; i < this.achievements.Count; i++)
				{
					...
					{
						if (Game._achievementKeyToIdMap == null)
						{
							...
							Game._achievementKeyToIdMap = dictionary;
						}
						int num;
						if (Game._achievementKeyToIdMap.TryGetValue(key, out num))
						{
							switch (num)
							{
							case 0:
		...
		protected override void LoadContent()
		{
			...
			{
				if (Game._languageCodeToIndexMap == null)
				{
					...
					dictionary.Add("es", 4);
					dictionary.Add("ja", 5);
					Game._languageCodeToIndexMap = dictionary;
				}
				int num;
				if (Game._languageCodeToIndexMap.TryGetValue(twoLetterISOLanguageName, out num))
				{
					switch (num)
					{
					case 1:
		...
		private static Dictionary<string, int> _achievementKeyToIdMap;
		private static Dictionary<string, int> _languageCodeToIndexMap;
